...
def insert_and_get_id(conn, sql, params=None):
    ...
    Uses OUTPUT INSERTED.id clause appended to the SQL.
    Works on both local SQL Server and Azure SQL.
    """
    target = conn._conn if isinstance(conn, Connection) else conn
    # Inject OUTPUT clause after INSERT to get the new ID
    # INSERT INTO table (...) OUTPUT INSERTED.id VALUES (...)
    insert_sql = sql.strip().rstrip(";")
    if "OUTPUT INSERTED" not in insert_sql.upper():
        insert_sql = insert_sql.replace("VALUES", "OUTPUT INSERTED.id VALUES", 1)
